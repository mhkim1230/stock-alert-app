...
@router.get("/stocks/search", response_model=SearchStocksResponse, dependencies=[Depends(require_admin_key)])
async def search_stocks(query: str):
    return {"results": await stock_service.search_stocks(query)}


@router.get("/stocks/{symbol}", response_model=StockQuoteResponse, dependencies=[Depends(require_admin_key)])
...
@router.get("/currency/rate", response_model=ExchangeRateResponse, dependencies=[Depends(require_admin_key)])
